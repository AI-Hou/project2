...
def fillna(data):   #缺失值填充  字符串类的填众数，数值型的填均值，
...
def property_feature(org):
    ...
    property_list=[]
    for i in tmp:
        property_list+=i   # 把所有属性全都放在一个列表里
    for i in property_list:   # 把所有的属性计数并放到字典中
